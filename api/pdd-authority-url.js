import { PddApiError, generateMemberAuthorityUrl, getPddEnvPresence } from './_lib/pdd.js';
...
function sendError(res, error) {
  ...
  console.error('[pdd-authority-url]', env);
...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    ...
    res.status(405).json({ error: 'method-not-allowed', message: 'Use POST /api/pdd-authority-url.' });
    return;
  }
  ...
  try {
    const result = await generateMemberAuthorityUrl();

    console.info('[pdd-authority-url]', env);
    ...
    res.status(200).json({
      ...
      we_app_info: result.we_app_info,
