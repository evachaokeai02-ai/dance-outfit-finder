...
const CATEGORY_DISPLAY_FALLBACK = {
  top: '上衣',
  bottom: '下装',
  shoes: '鞋子',
  accessory: '配饰',
...
const CATEGORY_DISPLAY_CANDIDATES = {
  top: ['短上衣', '短外套', '运动背心', '吊带', '卫衣', '开衫', '上衣'],
  bottom: ['百褶短裙', '短裙', '短裤', '工装裤', '阔腿裤', '皮裙', '下装'],
  shoes: ['厚底鞋', '跳舞鞋', '运动鞋', '短靴', '老爹鞋', '鞋子'],
  accessory: ['腰链', '腰带', '发带', '棒球帽', '项链', '配饰'],
};

const DISPLAY_TAG_CANDIDATES = [
  '甜酷',
  '辣妹',
  '女团风',
  '元气',
...
function pickDisplayTitle(goodsName, category) {
  const normalizedName = normalizeText(goodsName);
  const candidates = CATEGORY_DISPLAY_CANDIDATES[category] || [];
  const matched = candidates.find((candidate) => normalizedName.includes(candidate));
  return clampDisplayText(matched || CATEGORY_DISPLAY_FALLBACK[category] || '单品', 12);
}

function buildDisplayTags(product, query = {}) {
  const normalizedName = normalizeText(product.goodsName || product.goods_name || '');
  const fromQuery = [...(query.styleTags || []), ...(query.sceneTags || []), ...(query.bodyTags || []), ...(query.danceTags || [])];
  const tags = [...fromQuery, ...DISPLAY_TAG_CANDIDATES.filter((tag) => normalizedName.includes(tag))]
    .map((tag) => clampDisplayText(tag, 5))
...
function getPromotionParamsDebug() {
  return getPddDebug();
}


function getGoodsSearchDebug() {
  return getPddDebug();
}
...
export async function searchGoods({ keyword, page = DEFAULT_PAGE, pageSize = DEFAULT_PAGE_SIZE, sortType = 0, withCoupon = false }) {
  ...
  return {
    ...
    goodsListCount: Array.isArray(goodsList) ? goodsList.length : 0,
    products,
    pddDebug: searchParamsDebug,
  };
}
...
export function toRecommendationProduct(product, query, index = 0) {
  ...
  return {
    ...
    displayTitle: product.displayTitle || pickDisplayTitle(product.goodsName || query.keyword, query.category),
    displayTags: product.displayTags || buildDisplayTags(product, query),
    ...
    price: Number(product.couponPrice || product.price || 0),
    priceLabel: Number(product.couponPrice || product.price || 0) > 0 ? `¥${Number(product.couponPrice || product.price || 0).toFixed(2).replace(/\.00$/, '')}` : '',
    ...
    pdd: {
      ...
      fullGoodsName: product.goodsName || '',
      rawKeyword: query.keyword,
      searchQuery: query.keyword,
      queryUsed: query.keyword,
      queryAttempts: [query.keyword],
